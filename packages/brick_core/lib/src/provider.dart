...
abstract class Provider<_Model extends Model> {
  ...
  delete<T extends _Model>(T instance, {Query? query, required ModelRepository<_Model> repository});
  ...
  // ignore: always_declare_return_types
  exists<T extends _Model>({Query? query, required ModelRepository<_Model>? repository}) => null;

  /// Query for raw data and construct it with an [Adapter]
  // ignore: always_declare_return_types
  get<T extends _Model>({Query? query, required ModelRepository<_Model> repository});

  /// Insert or update a model instance
  // ignore: always_declare_return_types
  upsert<T extends _Model>(T instance, {Query? query, required ModelRepository<_Model> repository});
}
